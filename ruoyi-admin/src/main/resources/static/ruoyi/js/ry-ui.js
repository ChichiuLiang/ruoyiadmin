...
(function ($) {
    $.extend({
    	...
    	table: {
          ...
          //动态合并行跟列
    		mergeCells : function(data,fieldName,fieldName2,target,otherName){
                ...
                var colsSortMap = {};
                if(null != fieldName2){
                	//声明一个map计算相同属性值在data对象  行方向上出现的次数和
                    for(var i = 0 ; i < data.length ; i++){
                        for(var prop in data[i]){
                            if(prop == fieldName){            			
                       			var key1 = data[i][prop];
                       			var key2 = data[i][fieldName2];
                       			if(key1 == key2){
                       				colsSortMap[key1] = 2;
                       			}else{
                       				colsSortMap[key1] = 1;
                    ...
                    for(var prop in data[i]){
                        if(prop == fieldName){
                            ...
                            if(otherName != null && otherName.length > 0){
                            	...
                            	for(var value of otherName){
                            		$(target).bootstrapTable('mergeCells',{index:i, field:value, colspan: cols, rowspan: rows});
                            	}
                            }
