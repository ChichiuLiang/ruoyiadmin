    ...
    $h = {
        ...
        dataURI2Blob: function (dataURI) {
            ...
            var BlobBuilder = window.BlobBuilder || window.WebKitBlobBuilder || window.MozBlobBuilder ||
                window.MSBlobBuilder, canBlob = $h.hasBlobSupport(), byteStr, arrayBuffer, intArray,
                i, mimeStr, bb,
        ...
        arrayBuffer2String: function (buffer) {
            ...
            var array = Array.prototype.slice.apply(new Uint8Array(buffer)), out = '', i = 0, len,
                c, char2, char3;
            len = array.length;
            while (i < len) {
                c = array[i++];
    ...
    FileInput.prototype = {
        ...
        _initResumableUpload: function () {
            ...
            self.resumableManager = {
                ...
                upload: function () {
                    var rm = self.resumableManager, fm = self.fileManager, ids = fm.getIdList(),
                        flag = 'new',
                        intervalId;
                ...
                testUpload: function () {
                    var rm = self.resumableManager, opts = self.resumableUploadOptions, fd, f,
                        fm = self.fileManager, id = rm.id, fnBefore, fnSuccess, fnError, fnComplete,
                        outData;
                    if (!opts.testUrl) {
                        rm.testing = false;
                        return;
        ...
        _initTemplateDefaults: function () {
            var self = this, tMain1, tMain2, tPreview, tFileIcon, tClose, tCaption, tBtnDefault,
                tBtnLink, tBtnBrowse,
                tModalMain, tModal, tProgress, tSize, tFooter, tActions, tActionDelete,
                tActionUpload, tActionDownload,
                tActionZoom, tActionDrag, tIndicator, tTagBef, tTagBef1, tTagBef2, tTagAft,
                tGeneric, tHtml, tImage,
                tText, tOffice, tGdocs, tVideo, tAudio, tFlash, tObject, tPdf, tOther, tStyle,
                tZoomCache, vDefaultDim,
                tStats;
            tMain1 = '{preview}\n' +
            ...
            self.previewCache = {
                ...
                get: function (i, isDisabled) {
                    var ind = 'init_' + i, data = self.previewCache.data, config = data.config[i],
                        fileId,
                        content = data.content[i], previewId = self.previewInitId + '-' + ind, out,
                        $tmp, cat, ftr,
        ...
        _uploadSingle: function (i, id, isBatch) {
            var self = this, fm = self.fileManager, count = fm.count(), formdata = new FormData(),
                outData,
                previewId = self.previewInitId + '-' + i, $thumb, chkComplete, $btnUpload,
                $btnDelete,
        ...
        _uploadBatch: function () {
            var self = this, fm = self.fileManager, total = fm.total(), params = {}, fnBefore,
                fnSuccess, fnError,
        ...
        _initPreviewActions: function () {
            ...
            $preview.find(btnRemove).each(function () {
                ...
                var $frame = $el.closest($h.FRAMES), cache = self.previewCache.data, settings,
                    params, config,
        ...
        _generatePreviewTemplate: function (
            ...
            var self = this, caption = self.slug(fname), prevContent, zoomContent = '',
                ...
                config, newCat = self.preferIconicPreview ? 'other' : cat, title = caption,
                alt = caption,
        ...
        canPreview: function (file) {
            ...
            var name = file.name || '', type = file.type || '', size = (file.size || 0) / 1000,
                cat = self._parseFileType(type, name), allowedTypes, allowedMimes, allowedExts,
                skipPreview,
        ...
        _previewFile: function (i, file, theFile, previewId, data, fileInfo) {
            ...
            var self = this, fname = $h.getFileName(file), ftype = fileInfo.type,
                ...
                cat = self._parseFileType(ftype, fname), content, $preview = self.$preview,
                fsize = file.size || 0,
        ...
        _setInitThumbAttr: function () {
            var self = this, data = self.previewCache.data, len = self.previewCache.count(true),
                config,
        ...
        setImageOrientation: function ($img, $zoomImg, value, $thumb) {
            var self = this, invalidImg = !$img || !$img.length,
                ...
                isHidden = false, $div,
                zoomOnly = invalidImg && $thumb && $thumb.attr('data-template') === 'image', ev;
        ...
        _getResizedImage: function (id, config, counter, numImgs) {
            var self = this, img = $(config.img)[0], width = img.naturalWidth,
                ...
                dataURI,
                context = self.imageCanvasContext, type = config.typ, pid = config.pid,
                ind = config.ind,
                $thumb = config.thumb, throwError, msg, exifObj = config.exifObj, exifStr, file,
                params, evParams;
        ...
        _renderFileFooter: function (cat, caption, size, width, isError) {
            var self = this, config = self.fileActionSettings, rem = config.showRemove,
                drg = config.showDrag,
        ...
        _renderFileActions: function (
            ...
            var vUrl = url === false ? '' : ' data-url="' + url + '"', btnZoom = '', btnDrag = '',
                css,
        ...
        readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                var node = ctr + i, previewId = previewInitId + '-' + node, file = files[i],
                    fSizeKB, j, msg, $thumb,
                    fnText = settings.text, fnImage = settings.image, fnHtml = settings.html, typ,
                    chk, typ1, typ2,
                    caption = self._getFileName(file, ''),
        ...
        pause: function () {
            var self = this, rm = self.resumableManager, xhr = self.ajaxRequests, len = xhr.length,
                i,
        ...
        cancel: function () {
            var self = this, xhr = self.ajaxRequests, rm = self.resumableManager, len = xhr.length,
                i;
